...
# Objective   : Objective, gradient, hessian function of different methods.
...
#               (4) The naive EB, CEB and BCEB use and only use the firsr replicate dataset.
...
#               (5) The MGF of ME following a t distribution does not exist, and thus, we do not estimate the CEB estimator in this simulation.
...
objective.zqy.r_T <- function(theta) {
  f <- 0
  f <- f +
    log(sum(drop((1 - W) * exp(Ze_T[[1]] %*% theta)))) +
    sum(-Ze1_bar_T[[1]] * theta)
  return(f)
}

gradient.zqy.r_T <- function(theta) {
  f <- 0
  w <- drop((1 - W) * exp(Ze_T[[1]] %*% theta))
  f <- f + drop(w %*% (Ze_T[[1]]) / sum(w) - Ze1_bar_T[[1]])
  return(f)
}

hessian.zqy.r_T <- function(theta) {
  f <- 0
  w <- drop((1 - W) * exp(Ze_T[[1]] %*% theta))
  a <- matrix(rep(0, p_Z^2), nrow = p_Z, ncol = p_Z)
  for (j in 1:N) {
    a <- a + (Ze_T[[1]][j,]) %*% t(Ze_T[[1]][j,]) * w[j]
  }
  w2 <- a * sum(w) - t(w %*% Ze_T[[1]]) %*% (w %*% Ze_T[[1]])
  f <- f + drop(w2 / (sum(w))^2)
  return(f)
}
...
objective.ebme.r_T <- function(theta) {
  f <- drop(log((1 - W) %*% exp(Ze_T[[1]] %*% theta)) -
              Ze1_bar_T[[1]] %*% theta -
              t(theta) %*% Sigma %*% theta / 2)
  return(f)
}

gradient.ebme.r_T <- function(theta) {
  w <- drop((1 - W) * exp(Ze_T[[1]] %*% theta))
  f <- drop(w %*% Ze_T[[1]] / sum(w) - Ze1_bar_T[[1]]) - drop(Sigma %*% theta)
  return(f)
}
...
gradient.hcc.r_T <- function(theta) {
  eta0 <- 0
  eta1 <- 0
  for (u in 2:r) {
    for (s in 1:(u - 1)) {
      eta0 <- eta0 + sum(exp(drop((theta %*% t(Ze_T[[u]] - Ze_T[[s]])))) +
                           exp(drop((theta %*% t(Ze_T[[s]] - Ze_T[[u]])))))
      eta1 <- eta1 +
        (drop(t(Ze_T[[u]] - Ze_T[[s]]) %*% (exp(drop(theta %*% t(Ze_T[[u]] - Ze_T[[s]])))))) +
        (drop(t(Ze_T[[s]] - Ze_T[[u]]) %*% (exp(drop(theta %*% t(Ze_T[[s]] - Ze_T[[u]]))))))
    }
  }
  ...
  f1 <- 0
  for (k in 1:r) {
    f0 <- f0 + sum(drop((1 - W) * exp(Ze_T[[k]] %*% theta)))
    f1 <- f1 + drop((t(Ze_T[[k]]) - eta1 / eta0) %*% drop((1 - W) * exp(Ze_T[[k]] %*% theta)))
  }
  f <- drop(f1 / f0 - Ze1_bar_al_T)
  # f <- sum(f^2)
  return(f)
}
...
gradient.hyj.r_T <- function(theta) {

  index_Permutation <- permutations(n = r, r = 2, v = 1:r)
  N_Permutation <- nrow(index_Permutation)
  # RepDataset_Z <- list(repset1 = Ze_T[[1]], repset2 = Ze_T[[2]], repset3 = Ze_T[[3]])
  RepDataset_Z <- list(repset1 = Ze_T[[1]], repset2 = Ze_T[[2]])
  ...
  for (k in 1:r) {
    if (k == 1) {
      Denominator <- (1 - W) %*% exp(Ze_T[[k]] %*% theta)
    }else {
      Denominator <- Denominator + (1 - W) %*% exp(Ze_T[[k]] %*% theta)
    }
  }
  Denominator <- Denominator / r
  # 注意：后半部分Z1bar【不能】直接用colMeans！m1要除的是n1，不是N！
  HYJ_val <- Numerator / drop(Denominator) - Ze1_bar_al_T
  return(HYJ_val)
}
