# -*- coding: utf-8 -*-
# ------------------------------------------
# Title       : 998.TestFile for new Normal ME for estimating ATE
# Objective   : Updata Rcode of "Compare all methods with Normal measurement errors"
...

{
  ...
  ## MSE for theta1, theta2, theta3, theta4, ATE, ATE
  MSE <- function(x) {
    # trueVal <- c(True.theta[-1], ATE)
    colMeans((t(t(x) - trueVal))^2,na.rm = TRUE)
  }
  ...
  source(file = "./Simulation/Supplementary/TabS2&FigS1/Code/999.ObjectiveGradient_NormalME_v2.R")
  ...
  # Variable Information----
{
  ...
  vec_Vareps <- seq(0, 0.5, by = 0.1)
  # vec_Vareps <- c(0.1, 0.5)
...
{
{
  ...
  #### 7 cols: rhoZ, Var(eps), theta1, theta2, theta3, theta4, ATE
  #### nrow = N_Sim
  ...
  mat_avgSim_eb_Normal <- matrix(ncol = 7 - 4,
                                 nrow = length(vec_rhoZ) * length(vec_Vareps))
  ...
  #### 17 cols: rhoZ, Var(eps), bias(theta1, theta2, theta3, theta4, ATE), sd(theta1, theta2, theta3, theta4, ATE), MSE(theta1, theta2, theta3, theta4, ATE)
  #### nrow = length(vec_rhoZ) * length(vec_Vareps)
  mat_BiasSdMSE_eb_Normal <- matrix(ncol = 17 - 4 * 3,
                                    nrow = length(vec_rhoZ) * length(vec_Vareps))
  colnames(mat_BiasSdMSE_eb_Normal) <- c("rhoZ", "Var_eps",
                                         # paste0("BIAS_", c(paste0("theta", 1:4), "ATE")),
                                         # paste0("SD_", c(paste0("theta", 1:4), "ATE")),
                                         # paste0("MSE_", c(paste0("theta", 1:4), "ATE")),
                                         "BIAS_ATE", "SD_ATE", "MSE_ATE")
  ...
  # Add correlation to the covariance matrix of covariates
{
  ...
  for (var_eps in vec_Vareps) {
      ...
      # Mean value of observed Z1 (with measurement errors) in replicated dataset
    {
      ...
      # Mean value of observed Z0 (with measurement errors) in replicated dataset
    {
      ## Normal distributed measurement errors
      Ze0_bar_al_Normal <- rep(0, p_Z)
      for (k in 1:r) {
        Ze0_Normal[[k]] <- as.matrix(Ze_Normal[[k]][Index_col,])
        Ze0_bar_Normal[[k]] <- colMeans(Ze0_Normal[[k]])
        Ze0_bar_al_Normal <- Ze0_bar_al_Normal + Ze0_bar_Normal[[k]]
      }
      Ze0_bar_al_Normal <- Ze0_bar_al_Normal / r
    }
    ...
    {
      opt.out.zqy.r_Normal_ATT <- optim(par = rep(0, (ncol(Z))),
                                        fn = objective.zqy.r_Normal,
                                        gr = gradient.zqy.r_Normal,
                                        method = "BFGS",
                                        control = list(trace = 0, reltol = .Machine$double.eps, maxit = 200),
                                        ATT = TRUE)
      theta0_Normal_ATT <- opt.out.zqy.r_Normal_ATT$par
      # round(theta0_Normal, 3)

      opt.out.zqy.r_Normal_ATC <- optim(par = rep(0, (ncol(Z))),
                                        fn = objective.zqy.r_Normal,
                                        gr = gradient.zqy.r_Normal,
                                        method = "BFGS",
                                        control = list(trace = 0, reltol = .Machine$double.eps, maxit = 200),
                                        ATT = FALSE)
      theta0_Normal_ATC <- opt.out.zqy.r_Normal_ATC$par
    }

      # EBME - BFGS
    {
      output.CEB.solveObj_Normal_ATT <- tryCatch(optim(par = rep(0, p_Z),
      ...
      if (class(output.CEB.solveObj_Normal_ATT) != "list" |
        is.na(output.CEB.solveObj_Normal_ATT$value) |
        is.nan(output.CEB.solveObj_Normal_ATT$value)) {
        # 此时无法求解CEB，将naive EB的结果赋给CEB
        output.CEB.solveObj_Normal_ATT <- opt.out.zqy.r_Normal_ATT
      }
      theta_CEB_solveObj_ATT <- output.CEB.solveObj_Normal_ATT$par
      ObjValue_CEB_solveObj_ATT <- output.CEB.solveObj_Normal_ATT$value

      # theta_CEB - solve the gradient funtion
      output.CEB.solveGradient_Normal_ATT <- tryCatch(multiroot(f = gradient.ebme.r_Normal,
                                                                start = rep(0, p_Z),
      ...
      if (class(output.CEB.solveGradient_Normal_ATT) != "list") {
        # 此时求解CEB gradient 不收敛，直接赋值9999
        ObjValue_CEB_solveGradient_ATT <- 9999
      }else {
        theta_CEB_solveGradient_ATT <- output.CEB.solveGradient_Normal_ATT$root
        ObjValue_CEB_solveGradient_ATT <- objective.ebme.r_Normal(theta = theta_CEB_solveGradient_ATT)
        if (is.nan(ObjValue_CEB_solveGradient_ATT) | is.na(ObjValue_CEB_solveGradient_ATT)) { ObjValue_CEB_solveGradient_ATT <- 9999 }
      }

      if (ObjValue_CEB_solveObj_ATT <= ObjValue_CEB_solveGradient_ATT) {
        theta_CEB_Normal_ATT <- theta_CEB_solveObj_ATT
        iterbfgsind.r_Normal[i] <- (max(abs(gradient.ebme.r_Normal(theta_CEB_solveObj_ATT))) < 1e-04)
        ## double check
        ...
        theta_CEB_Normal_ATT <- theta_CEB_solveGradient_ATT
        iterbfgsind.r_Normal[i] <- (max(abs(output.CEB.solveGradient_Normal_ATT$f.root)) < sqrt(.Machine$double.eps)) & (output.CEB.solveGradient_Normal_ATT$estim.precis != "NaN")
        ## double check
        if (is.na(iterbfgsind.r_Normal[i]) | is.nan(iterbfgsind.r_Normal[i])) {
          iterbfgsind.r_Normal[i] <- FALSE
          print("Solution to CEB fail to converge!")
      ...
      output.CEB.solveObj_Normal_ATC <- tryCatch(optim(par = rep(0, p_Z),
                                                       fn = objective.ebme.r_Normal,
                                                       gr = gradient.ebme.r_Normal,
                                                       method = "BFGS",
                                                       control = list(trace = 0, reltol = .Machine$double.eps, maxit = 200),
      ...
      # 如果optim无法求解，output.CEB.solveObj_Normal会是一个logical变量；否则，如果optim能求解，output.CEB.solveObj_Normal是一个list变量
      if (class(output.CEB.solveObj_Normal_ATC) != "list" |
        is.na(output.CEB.solveObj_Normal_ATC$value) |
        is.nan(output.CEB.solveObj_Normal_ATC$value)) {
        # 此时无法求解CEB，将naive EB的结果赋给CEB
        output.CEB.solveObj_Normal_ATC <- opt.out.zqy.r_Normal_ATC
      }
      theta_CEB_solveObj_ATC <- output.CEB.solveObj_Normal_ATC$par
      ObjValue_CEB_solveObj_ATC <- output.CEB.solveObj_Normal_ATC$value

      # theta_CEB - solve the gradient funtion
      output.CEB.solveGradient_Normal_ATC <- tryCatch(multiroot(f = gradient.ebme.r_Normal,
                                                                start = rep(0, p_Z),
                                                                rtol = .Machine$double.eps, maxiter = 200,
                                                                ATT = FALSE),
                                                      error = function(e) {
      ...
      # 如果multiroot无法求解，output.CEB.solveGradient会是一个logical变量；否则，如果multiroot能求解，output.CEB.solveGradient是一个list变量
      if (class(output.CEB.solveGradient_Normal_ATC) != "list") {
        # 此时求解CEB gradient 不收敛，直接赋值9999
        ObjValue_CEB_solveGradient_ATC <- 9999
      }else {
        theta_CEB_solveGradient_ATC <- output.CEB.solveGradient_Normal_ATC$root
        ObjValue_CEB_solveGradient_ATC <- objective.ebme.r_Normal(theta = theta_CEB_solveGradient_ATC, ATT = FALSE)
        if (is.nan(ObjValue_CEB_solveGradient_ATC) | is.na(ObjValue_CEB_solveGradient_ATC)) { ObjValue_CEB_solveGradient_ATC <- 9999 }
      }

      if (ObjValue_CEB_solveObj_ATC <= ObjValue_CEB_solveGradient_ATC) {
        theta_CEB_Normal_ATC <- theta_CEB_solveObj_ATC
        iterbfgsind.r_Normal[i] <- (max(abs(gradient.ebme.r_Normal(theta_CEB_solveObj_ATC))) < 1e-04)
        ## double check
        if (is.na(iterbfgsind.r_Normal[i]) | is.nan(iterbfgsind.r_Normal[i])) {
          iterbfgsind.r_Normal[i] <- FALSE
          print("Solution to CEB fail to converge!")
        ...
        theta_CEB_Normal_ATC <- theta_CEB_solveGradient_ATC
        iterbfgsind.r_Normal[i] <- (max(abs(output.CEB.solveGradient_Normal_ATC$f.root)) < sqrt(.Machine$double.eps)) & (output.CEB.solveGradient_Normal_ATC$estim.precis != "NaN")
        ## double check
        if (is.na(iterbfgsind.r_Normal[i]) | is.nan(iterbfgsind.r_Normal[i])) {
          iterbfgsind.r_Normal[i] <- FALSE
          print("Solution to CEB fail to converge!")
    ...
    {
      ## Normal distributed measurement errors
      theta_bc_Normal_ATT <- drop(ginv(hessian.zqy.r_Normal(theta0_Normal_ATT, ATT = TRUE) - Sigma) %*%
                                    hessian.zqy.r_Normal(theta0_Normal_ATT, ATT = TRUE) %*%
                                    theta0_Normal_ATT)
      ### 判断BC是否无法求解：（默认【收敛】）存在以下情况任意其一都被判为【无解/不收敛】
      iterbcind.r_Normal[i] <- !(NaN %in% theta_bc_Normal_ATT | (sum(is.infinite(theta_bc_Normal_ATT)) > 0))

      ## Normal distributed measurement errors
      theta_bc_Normal_ATC <- drop(ginv(hessian.zqy.r_Normal(theta0_Normal_ATC, ATT = FALSE) - Sigma) %*%
                                    hessian.zqy.r_Normal(theta0_Normal_ATC, ATT = FALSE) %*%
                                    theta0_Normal_ATC)
    }
    ...
    {
      ## Normal distributed measurement errors
      opt.out.hcc.r_Normal_ATT <- multiroot(f = gradient.hcc.r_Normal, ATT = TRUE, start = rep(0, p_Z),
                                            rtol = .Machine$double.eps, maxiter = 200)
      iterhccind.r_Normal[i] <- (max(gradient.hcc.r_Normal(opt.out.hcc.r_Normal_ATT$root)) < sqrt(.Machine$double.eps)) & (opt.out.hcc.r_Normal_ATT$estim.precis != "NaN")

      ## Normal distributed measurement errors
      opt.out.hcc.r_Normal_ATC <- multiroot(f = gradient.hcc.r_Normal, ATT = FALSE, start = rep(0, p_Z),
                                            rtol = .Machine$double.eps, maxiter = 200)
    }
    ...
    {
      ## Normal distributed measurement errors
      opt.out.hyj.r_Normal_ATT <- multiroot(f = gradient.hyj.r_Normal, start = rep(0, p_Z),
                                            rtol = .Machine$double.eps, maxiter = 200,
                                            ATT = TRUE)
      iterhyjind.r_Normal[i] <- (max(gradient.hyj.r_Normal(opt.out.hyj.r_Normal_ATT$root)) < sqrt(.Machine$double.eps)) & (opt.out.hyj.r_Normal_ATT$estim.precis != "NaN")
      ### double check
      ...
      ## Normal distributed measurement errors
      opt.out.hyj.r_Normal_ATC <- multiroot(f = gradient.hyj.r_Normal, start = rep(0, p_Z),
                                            rtol = .Machine$double.eps, maxiter = 200,
                                            ATT = FALSE)
    }

      # ATE estimation with \hat{\theta}
    {
      # ZQY-EB: naive Entropy Balancing
    {
      # ATC
      weight_zqy_Normal_ATC <- exp(Ze1_Normal[[1]] %*% theta0_Normal_ATC)
      weight_zqy_Normal_ATC <- weight_zqy_Normal_ATC / sum(weight_zqy_Normal_ATC)
      ATC_zqy_Normal <- t(weight_zqy_Normal_ATC) %*% Y1 - mean(Y0)

      # ATT
      weight_zqy_Normal_ATT <- exp(Ze0_Normal[[1]] %*% theta0_Normal_ATT)
      weight_zqy_Normal_ATT <- weight_zqy_Normal_ATT / sum(weight_zqy_Normal_ATT)
      ATT_zqy_Normal <- mean(Y1) - t(weight_zqy_Normal_ATT) %*% Y0

      # ATE
      ATE_zqy_Normal <- n1 / N * ATT_zqy_Normal + n0 / N * ATC_zqy_Normal

      mat_SingleSim_eb_Normal[i, "ATE"] <- mat_SingleSim_eb_Normal[i, "ATE"] + ATE_zqy_Normal
    }

      # CEB / EBME - bfgs C2: the proposed method
    {
      # ATC
      weight_bfgs_Normal_ATC <- exp(Ze1_Normal[[1]] %*% theta_CEB_Normal_ATC)
      weight_bfgs_Normal_ATC <- weight_bfgs_Normal_ATC / sum(weight_bfgs_Normal_ATC)
      ATC_bfgs_Normal <- t(weight_bfgs_Normal_ATC) %*% Y1 - mean(Y0)

      # ATT
      weight_bfgs_Normal_ATT <- exp(Ze0_Normal[[1]] %*% theta_CEB_Normal_ATT)
      weight_bfgs_Normal_ATT <- weight_bfgs_Normal_ATT / sum(weight_bfgs_Normal_ATT)
      ATT_bfgs_Normal <- mean(Y1) - t(weight_bfgs_Normal_ATT) %*% Y0

      # ATE
      ATE_bfgs_Normal <- n1 / N * ATT_bfgs_Normal + n0 / N * ATC_bfgs_Normal

      mat_SingleSim_ebme_Normal[i, "ATE"] <- mat_SingleSim_ebme_Normal[i, "ATE"] + ATE_bfgs_Normal
    }

      # BCEB
    {
      # ATC
      weight_bc_Normal_ATC <- exp(Ze1_Normal[[1]] %*% theta_bc_Normal_ATC)
      weight_bc_Normal_ATC <- weight_bc_Normal_ATC / sum(weight_bc_Normal_ATC)
      ATC_bc_Normal <- t(weight_bc_Normal_ATC) %*% Y1 - mean(Y0)

      # ATT
      weight_bc_Normal_ATT <- exp(Ze0_Normal[[1]] %*% theta_bc_Normal_ATT)
      weight_bc_Normal_ATT <- weight_bc_Normal_ATT / sum(weight_bc_Normal_ATT)
      ATT_bc_Normal <- mean(Y1) - t(weight_bc_Normal_ATT) %*% Y0

      # ATE
      ATE_bc_Normal <- n1 / N * ATT_bc_Normal + n0 / N * ATC_bc_Normal

      mat_SingleSim_bc_Normal[i, "ATE"] <- mat_SingleSim_bc_Normal[i, "ATE"] + ATE_bc_Normal
    }

      # HCC
    {
      par_hcc_Normal_ATC <- as.matrix(opt.out.hcc.r_Normal_ATC$root, ncol = 1)
      par_hcc_Normal_ATT <- as.matrix(opt.out.hcc.r_Normal_ATT$root, ncol = 1)

      # ATC
      weight_hcc_Normal_ATC <- matrix(rep(0, n1))
      for (k in 1:r) {
        weight_hcc_Normal_ATC <- weight_hcc_Normal_ATC + exp(Ze1_Normal[[k]] %*% par_hcc_Normal_ATC)
      }
      weight_hcc_Normal_ATC <- weight_hcc_Normal_ATC / sum(weight_hcc_Normal_ATC)
      ATC_hcc_Normal <- t(weight_hcc_Normal_ATC) %*% Y1 - mean(Y0)

      # ATT
      weight_hcc_Normal_ATT <- matrix(rep(0, n0))
      for (k in 1:r) {
        weight_hcc_Normal_ATT <- weight_hcc_Normal_ATT + exp(Ze0_Normal[[k]] %*% par_hcc_Normal_ATT)
      }
      weight_hcc_Normal_ATT <- weight_hcc_Normal_ATT / sum(weight_hcc_Normal_ATT)
      ATT_hcc_Normal <- mean(Y1) - t(weight_hcc_Normal_ATT) %*% Y0

      # ATE
      ATE_hcc_Normal <- n1 / N * ATT_hcc_Normal + n0 / N * ATC_hcc_Normal

      mat_SingleSim_hcc_Normal[i, "ATE"] <- mat_SingleSim_hcc_Normal[i, "ATE"] + ATE_hcc_Normal
    }

      # HYJ
    {
      par_hyj_Normal_ATC <- as.matrix(opt.out.hyj.r_Normal_ATC$root, ncol = 1)
      par_hyj_Normal_ATT <- as.matrix(opt.out.hyj.r_Normal_ATT$root, ncol = 1)

      # ATC
      weight_hyj_Normal_ATC <- matrix(rep(0, n1))
      for (k in 1:r) {
        weight_hyj_Normal_ATC <- weight_hyj_Normal_ATC + exp(Ze1_Normal[[k]] %*% par_hyj_Normal_ATC)
      }
      weight_hyj_Normal_ATC <- weight_hyj_Normal_ATC / sum(weight_hyj_Normal_ATC)
      ATC_hyj_Normal <- t(weight_hyj_Normal_ATC) %*% Y1 - mean(Y0)

      # ATT
      weight_hyj_Normal_ATT <- matrix(rep(0, n0))
      for (k in 1:r) {
        weight_hyj_Normal_ATT <- weight_hyj_Normal_ATT + exp(Ze0_Normal[[k]] %*% par_hyj_Normal_ATT)
      }
      weight_hyj_Normal_ATT <- weight_hyj_Normal_ATT / sum(weight_hyj_Normal_ATT)
      ATT_hyj_Normal <- mean(Y1) - t(weight_hyj_Normal_ATT) %*% Y0

      # ATE
      ATE_hyj_Normal <- n1 / N * ATT_hyj_Normal + n0 / N * ATC_hyj_Normal

      mat_SingleSim_hyj_Normal[i, "ATE"] <- mat_SingleSim_hyj_Normal[i, "ATE"] + ATE_hyj_Normal
    }
      ...
      #   mat_SingleSim_ASMD_naiveEB[i,] <- unlist(ASMDCal(weight_zqy_Normal_ATT))
      #   mat_SingleSim_ASMD_CEB[i,] <- unlist(ASMDCal(weight_bfgs_Normal_ATT))
      #   mat_SingleSim_ASMD_BCEB[i,] <- unlist(ASMDCal(weight_bc_Normal_ATT))
      #   mat_SingleSim_ASMD_HL[i,] <- unlist(ASMDCal(weight_hcc_Normal_ATT))
      #   mat_SingleSim_ASMD_HW[i,] <- unlist(ASMDCal(weight_hyj_Normal_ATT))
      # }
  ...
  {
    mat_avgSim_eb_Normal[ith_vareps_rhoZ,] <- colMeans(mat_SingleSim_eb_Normal, na.rm = TRUE)
    mat_avgSim_ebme_Normal[ith_vareps_rhoZ,] <- colMeans(mat_SingleSim_ebme_Normal[iterbfgsind.r_Normal,], na.rm = TRUE)
    mat_avgSim_bc_Normal[ith_vareps_rhoZ,] <- colMeans(mat_SingleSim_bc_Normal[iterbcind.r_Normal,], na.rm = TRUE)
    mat_avgSim_hcc_Normal[ith_vareps_rhoZ,] <- colMeans(mat_SingleSim_hcc_Normal[iterhccind.r_Normal,], na.rm = TRUE)
    mat_avgSim_hyj_Normal[ith_vareps_rhoZ,] <- colMeans(mat_SingleSim_hyj_Normal[iterhyjind.r_Normal,], na.rm = TRUE)
  }
    ...
    trueVal <- ATE
  {
    ...
    ## bias
    # ColnamesinNeed_avgmat <- c(paste0("theta", 1:p_Z), "ATE")
    ColnamesinNeed_avgmat <- "ATE"
    # ColnamesinNeed_Bias <- paste0("BIAS_", c(paste0("theta", 1:p_Z), "ATE"))
    ColnamesinNeed_Bias <- "BIAS_ATE"
    ...
    ## sd
    # ColnamesinNeed_SD <- paste0("SD_", c(paste0("theta", 1:p_Z), "ATE"))
    ColnamesinNeed_SD <- "SD_ATE"
    ...
    mat_BiasSdMSE_eb_Normal[ith_vareps_rhoZ, ColnamesinNeed_SD] <- sd(mat_SingleSim_eb_Normal[, ColnamesinNeed_avgmat], na.rm = TRUE)
    mat_BiasSdMSE_ebme_Normal[ith_vareps_rhoZ, ColnamesinNeed_SD] <- sd(mat_SingleSim_ebme_Normal[iterbfgsind.r_Normal, ColnamesinNeed_avgmat], na.rm = TRUE)
    mat_BiasSdMSE_bc_Normal[ith_vareps_rhoZ, ColnamesinNeed_SD] <- sd(mat_SingleSim_bc_Normal[iterbcind.r_Normal, ColnamesinNeed_avgmat], na.rm = TRUE)
    mat_BiasSdMSE_hcc_Normal[ith_vareps_rhoZ, ColnamesinNeed_SD] <- sd(mat_SingleSim_hcc_Normal[iterhccind.r_Normal, ColnamesinNeed_avgmat], na.rm = TRUE)
    mat_BiasSdMSE_hyj_Normal[ith_vareps_rhoZ, ColnamesinNeed_SD] <- sd(mat_SingleSim_hyj_Normal[iterhyjind.r_Normal, ColnamesinNeed_avgmat], na.rm = TRUE)
    ## MSE
    # ColnamesinNeed_MSE <- paste0("MSE_", c(paste0("theta", 1:p_Z), "ATE"))
    ColnamesinNeed_MSE <- "MSE_ATE"
  ...
  # Table output----
{
  ...
  mat_Rc_Normal <- rbind(Rc_ebme_Normal,
                         ...
                         Rc_hcc_Normal,
                         Rc_hyj_Normal)
  csvName_mat_Rc_Normal <- paste0("mat_NormalEps_ConvergenceRate_N",
                                  N / 1000, "k_Sim",
                                  N_Sim / 1000, "k.xlsx")
  openxlsx::write.xlsx(x = as.data.frame(mat_Rc_Normal),
                       file = csvName_mat_Rc_Normal,
                       row.names = FALSE)
  ...
  mat_avgSim_Normal[, "ATE"] <- sprintf("%.3f", as.numeric(mat_avgSim_Normal[, "ATE"]))
  csvName_mat_avgSim <- paste0("mat_NormalEps_avgSim_theta_ATE_N",
                               N / 1000, "k_Sim",
                               N_Sim / 1000, "k.xlsx")
  openxlsx::write.xlsx(x = as.data.frame(mat_avgSim_Normal),
                       file = csvName_mat_avgSim,
                       row.names = FALSE)
  ...
  mat_BiasSdMSE_Normal[, "BIAS_ATE"] <- sprintf("%.3f", as.numeric(mat_BiasSdMSE_Normal[, "BIAS_ATE"]))
  mat_BiasSdMSE_Normal[, "SD_ATE"] <- sprintf("%.3f", as.numeric(mat_BiasSdMSE_Normal[, "SD_ATE"]))
  mat_BiasSdMSE_Normal[, "MSE_ATE"] <- sprintf("%.3f", as.numeric(mat_BiasSdMSE_Normal[, "MSE_ATE"]))
  csvName_mat_BiasSdMSE <- paste0("mat_NormalEps_BiasSdMSE_theta_ATE_N",
                                  N / 1000, "k_Sim",
                                  N_Sim / 1000, "k.xlsx")
  openxlsx::write.xlsx(x = as.data.frame(mat_BiasSdMSE_Normal),
                       file = csvName_mat_BiasSdMSE,
                       row.names = FALSE)
}
  ...
  # Plots----
{
  ...
  # mat_bias_Normal <- mat_BiasSdMSE_Normal[, c("rhoZ", "Var_eps", paste0("BIAS_", c(paste0("theta", 1:4), "ATE")), "Method")]
  # mat_sd_Normal <- mat_BiasSdMSE_Normal[, c("rhoZ", "Var_eps", paste0("SD_", c(paste0("theta", 1:4), "ATE")), "Method")]
  # mat_MSE_Normal <- mat_BiasSdMSE_Normal[, c("rhoZ", "Var_eps", paste0("MSE_", c(paste0("theta", 1:4), "ATE")), "Method")]

  mat_bias_Normal <- mat_BiasSdMSE_Normal[, c("rhoZ", "Var_eps", "BIAS_ATE", "Method")]
  mat_sd_Normal <- mat_BiasSdMSE_Normal[, c("rhoZ", "Var_eps", "SD_ATE", "Method")]
  mat_MSE_Normal <- mat_BiasSdMSE_Normal[, c("rhoZ", "Var_eps", "MSE_ATE", "Method")]
  ## bias
  ...
  # ### bias_theta
  # biasdf_theta_Normal <- biasdf_Normal[biasdf_Normal$variable != "BIAS_ATE",]
  ...
  #        units = "cm")
  ### bias_ATE
  biasdf_ATE_Normal <- biasdf_Normal[biasdf_Normal$variable == "BIAS_ATE",]
  p_AllMethods_bias_ATE_NormalEps <- ggplot(biasdf_ATE_Normal) +
    # facet_wrap(~rhoZ) +  # 行按rhoZ进行分区
    geom_line(aes(x = Var_eps, y = value, color = Method, group = Method)) +
    geom_point(aes(x = Var_eps, y = value, color = Method, shape = Method)) +
    # labs(x = "Variance of Measurement Error", y = "Bias", title = "Bias of ATE with Normal Measurement Errors") +
    labs(x = "Variance of Measurement Error", y = "Bias") +
    geom_abline(slope = 0, intercept = 0, lty = 3, lwd = 0.5) +
    # ylim(-9, 2) +
    # scale_y_continuous(breaks = seq(-9, 2, by = 0.5)) +
    theme_bw()
  p_AllMethods_bias_ATE_NormalEps
  ggsave(filename = "p_NormalEps_AllMethods_bias_ATE.pdf", p_AllMethods_bias_ATE_NormalEps,
         width = 26, height = 10,
         units = "cm")
  ## sd
  ...
  # ### sd_theta
  # sddf_theta_Normal <- sddf_Normal[sddf_Normal$variable != "SD_ATE",]
  ...
  #        units = "cm")
  ### sd_ATE
  sddf_ATE_Normal <- sddf_Normal[sddf_Normal$variable == "SD_ATE",]
  p_AllMethods_sd_ATE_NormalEps <- ggplot(sddf_ATE_Normal) +
    # facet_wrap(~rhoZ) +  # 行按rhoZ进行分区
    geom_line(aes(x = Var_eps, y = value, color = Method, group = Method)) +
    geom_point(aes(x = Var_eps, y = value, color = Method, shape = Method)) +
    # scale_y_continuous(limits = c(0, 2.25), breaks = seq(from = 0, to = 2.25, by = 0.5)) +
    scale_y_continuous(limits = c(0, max(sddf_ATE_Normal$value)), breaks = seq(from = 0, to = ceiling(max(sddf_ATE_Normal$value)), by = 0.5)) +
    # labs(x = "Variance of Measurement Error", y = "Standard Deviation", title = "SD of ATE with Normal Measurement Errors") +
    labs(x = "Variance of Measurement Error", y = "Standard Deviation") +
    geom_abline(slope = 0, intercept = 0, lty = 3, lwd = 0.5) +
    theme_bw()
  p_AllMethods_sd_ATE_NormalEps
  ggsave(filename = "p_NormalEps_AllMethods_sd_ATE.pdf", p_AllMethods_sd_ATE_NormalEps,
         width = 26, height = 10,
         units = "cm")
  ## MSE
  ...
  # ### MSE_theta
  # MSEdf_theta_Normal <- MSEdf_Normal[MSEdf_Normal$variable != "MSE_ATE",]
  ...
  #        units = "cm")
  ### MSE_ATE
  MSEdf_ATE_Normal <- MSEdf_Normal[MSEdf_Normal$variable == "MSE_ATE",]
  p_AllMethods_MSE_ATE_NormalEps <- ggplot(MSEdf_ATE_Normal) +
    # facet_wrap(~rhoZ) +  # 行按rhoZ进行分区
    geom_line(aes(x = Var_eps, y = value, color = Method, group = Method)) +
    geom_point(aes(x = Var_eps, y = value, color = Method, shape = Method)) +
    # labs(x = "Variance of Measurement Error", y = "Mean Squared Error", title = "MSE of ATE with Normal Measurement Errors") +
    labs(x = "Variance of Measurement Error", y = "Mean Squared Error") +
    geom_abline(slope = 0, intercept = 0, lty = 3, lwd = 0.5) +
    theme_bw()
  p_AllMethods_MSE_ATE_NormalEps
  ggsave(filename = "p_NormalEps_AllMethods_MSE_ATE.pdf", p_AllMethods_MSE_ATE_NormalEps,
         width = 26, height = 10,
         units = "cm")

  Figure_NormalEps_BiasSDMSE_ATE <- ggarrange(p_AllMethods_bias_ATE_NormalEps,
                                              p_AllMethods_sd_ATE_NormalEps,
                                              p_AllMethods_MSE_ATE_NormalEps,
                                              nrow = 1, ncol = 3,
  ...
  Figure_NormalEps_BiasSDMSE_ATE
}
}
...
{
  # Plot of bias+SD+MSE of ATE - Normal
  Figure_NormalEps_BiasSDMSE_ATE <- ggarrange(p_AllMethods_bias_ATE_NormalEps,
                                              p_AllMethods_sd_ATE_NormalEps,
                                              p_AllMethods_MSE_ATE_NormalEps,
                                              # nrow = 3, ncol = 3,
  ...
  ggsave(filename = "p_NormalEps_BiasSDMSE_ATE.pdf",
         Figure_NormalEps_BiasSDMSE_ATE,
         width = 33, height = 12,
         units = "cm")
}
