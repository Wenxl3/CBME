...
# Remark      : The Multiplicative Error.
# ------------------------------------------
...

{
  ...
  source(file = "./Simulation/Supplementary/TabS4/Code/CaseVI_MultiplicativeError/999.ObjectiveGradient_NormalME.R")
  ...
  # Add correlation to the covariance matrix of covariates
{
  ...
  for (var_eps in vec_Vareps) {
      ...
      # BCEB
    {
      ...
      iterbcind.r_Normal[i] <- !(is.na(ATT_bc_Normal) | is.na(mat_SingleSim_bc_Normal[i, "ATT"]))
    }
      ...
      # HCC
    {
      ...
      iterhccind.r_Normal[i] <- !(is.na(ATT_hcc_Normal) | is.na(mat_SingleSim_hcc_Normal[i, "ATT"]))
    }
      ...
      # HYJ
    {
      ...
      iterhyjind.r_Normal[i] <- !(is.na(ATT_hyj_Normal) | is.na(mat_SingleSim_hyj_Normal[i, "ATT"]))
    }
...
# Save .Rdata
save.image(file = "CompareAllMethods_MultiplicativeError_Original.RData")
  ...
  # Table output----
{
  ...
  mat_Rc_Normal <- rbind(Rc_ebme_Normal,
                         ...
                         Rc_hcc_Normal,
                         Rc_hyj_Normal)
  csvName_mat_Rc_Normal <- paste0("mat_MultiplicativeError_ConvergenceRate_N", N,
                                  "_Sim", N_Sim, ".csv")
  ...
  csvName_mat_avgSim <- paste0("mat_MultiplicativeError_avgSim_theta_ATT_N", N, "_Sim", N_Sim,
                               ".csv")
  ...
  csvName_mat_BiasSdMSE <- paste0("mat_MultiplicativeError_BiasSdMSE_theta_ATT_N", N, "_Sim", N_Sim,
                                  ".csv")
  ...
  # Plots----
{
  ...
  p_Rc_NormalEps <- ggplot(Rcdf_Normal) +
    ...
    theme_bw()
  p_Rc_NormalEps
  ggsave(filename = "p_MultiplicativeError_Rc.pdf", p_Rc_NormalEps,
         width = 14, height = 13,
         units = "cm")
  ...
  ggsave(filename = "p_MultiplicativeError_AllMethods_bias_theta.pdf", p_AllMethods_bias_theta_NormalEps,
         width = 26, height = 26,
         units = "cm")
  ### bias_ATT
  ...
  p_AllMethods_bias_ATT_NormalEps <- ggplot(biasdf_ATT_Normal) +
    ...
    geom_line(aes(x = Var_eps, y = value, color = Method, group = Method)) +
    geom_point(aes(x = Var_eps, y = value, color = Method, shape = Method)) +
    # labs(x = "Variance of Measurement Error", y = "Bias", title = "Bias of ATT with Normal Measurement Errors") +
  ...
  ggsave(filename = "p_MultiplicativeError_AllMethods_bias_ATT.pdf", p_AllMethods_bias_ATT_NormalEps,
         width = 26, height = 10,
         units = "cm")
  ## sd
  ...
  ggsave(filename = "p_MultiplicativeError_AllMethods_sd_theta.pdf", p_AllMethods_sd_theta_NormalEps,
         width = 26, height = 26,
         units = "cm")
  ### sd_ATT
  ...
  p_AllMethods_sd_ATT_NormalEps <- ggplot(sddf_ATT_Normal) +
    ...
    scale_y_continuous(limits = c(0, max(sddf_ATT_Normal$value)), breaks = seq(from = 0, to = ceiling(max(sddf_ATT_Normal$value)), by = 5)) +
  ...
  ggsave(filename = "p_MultiplicativeError_AllMethods_sd_ATT.pdf", p_AllMethods_sd_ATT_NormalEps,
         width = 26, height = 10,
         units = "cm")
  ## MSE
  ...
  ggsave(filename = "p_MultiplicativeError_AllMethods_MSE_theta.pdf", p_AllMethods_MSE_theta_NormalEps,
         width = 26, height = 26,
         units = "cm")
  ### MSE_ATT
  ...
  p_AllMethods_MSE_ATT_NormalEps <- ggplot(MSEdf_ATT_Normal) +
    ...
    geom_line(aes(x = Var_eps, y = value, color = Method, group = Method)) +
    geom_point(aes(x = Var_eps, y = value, color = Method, shape = Method)) +
    # labs(x = "Variance of Measurement Error", y = "Mean Squared Error", title = "MSE of ATT with Normal Measurement Errors") +
    labs(x = "Variance of Measurement Error", y = "Mean Squared Error") +
    geom_abline(slope = 0, intercept = 0, lty = 3, lwd = 0.5) +
    theme_bw()
  p_AllMethods_MSE_ATT_NormalEps
  ggsave(filename = "p_MultiplicativeError_AllMethods_MSE_ATT.pdf", p_AllMethods_MSE_ATT_NormalEps,
         width = 26, height = 10,
         units = "cm")
...
# Save .Rdata
save.image(file = "CompareAllMethods_MultiplicativeError_avgEdited.RData")
...
# Figure Plot: Combine the plot of normal distributed and Normal distribution----
{
  ...
  ggsave(filename = "p_MultiplicativeError_BiasSDMSE_ATT.pdf",
         Figure_NormalEps_BiasSDMSE_ATT,
         width = 33, height = 12,
         units = "cm")
}
...
openxlsx::write.xlsx(x = list_of_ASMD_NormalME,
                     file = "list_of_ASMD_MultiplicativeError.xlsx",
                     row.names = FALSE)
...
openxlsx::write.xlsx(x = mat_ASMD_AllMethods,
                     file = "mat_ASMD_AllMethods_MultiplicativeError.xlsx",
                     row.names = TRUE)