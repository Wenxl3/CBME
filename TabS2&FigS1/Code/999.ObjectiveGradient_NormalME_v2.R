# -*- coding: utf-8 -*-
# ------------------------------------------
# Title       : 999.ObjectiveGradient_NormalME
# Objective   : Construct the objective, gradient, hessian functions of estimating methods.
...
#               (4) The naive EB, CEB and BCEB only use the first replicate dataset.
...
objective.zqy.r_Normal <- function(theta, ATT = TRUE) {
  if (ATT) {
    f <- log((1 - W) %*% exp(Ze_Normal[[1]] %*% theta)) -
      Ze1_bar_Normal[[1]] %*% theta
  }else {
    f <- log(W %*% exp(Ze_Normal[[1]] %*% theta)) -
      Ze0_bar_Normal[[1]] %*% theta
  }
  return(f)
}

gradient.zqy.r_Normal <- function(theta, ATT = TRUE) {
  if (ATT) {
    w <- drop((1 - W) * exp(Ze_Normal[[1]] %*% theta))
    f <- drop(w %*% (Ze_Normal[[1]]) / sum(w) - Ze1_bar_Normal[[1]])
  }else {
    w <- drop(W * exp(Ze_Normal[[1]] %*% theta))
    f <- drop(w %*% (Ze_Normal[[1]]) / sum(w) - Ze0_bar_Normal[[1]])
  }
  return(f)
}

hessian.zqy.r_Normal <- function(theta, ATT = TRUE) {
  f <- 0
  if (ATT) {
    w <- drop((1 - W) * exp(Ze_Normal[[1]] %*% theta))
  }else {
...
objective.ebme.r_Normal <- function(theta, ATT = TRUE) {
  if (ATT) {
    f <- drop(log((1 - W) %*% exp(Ze_Normal[[1]] %*% theta)) -
                Ze1_bar_Normal[[1]] %*% theta -
                t(theta) %*% Sigma %*% theta / 2)
  }else {
    f <- drop(log(W %*% exp(Ze_Normal[[1]] %*% theta)) -
                Ze0_bar_Normal[[1]] %*% theta -
                t(theta) %*% Sigma %*% theta / 2)
  }
  return(f)
...
gradient.ebme.r_Normal <- function(theta, ATT = TRUE) {
  f <- 0
  if (ATT) {
    w <- drop((1 - W) * exp(Ze_Normal[[1]] %*% theta))
    f <- f + drop(w %*% Ze_Normal[[1]] / sum(w) - Ze1_bar_Normal[[1]]) - drop(Sigma %*% theta)
  }else {
    w <- drop(W * exp(Ze_Normal[[1]] %*% theta))
    f <- f + drop(w %*% Ze_Normal[[1]] / sum(w) - Ze0_bar_Normal[[1]]) - drop(Sigma %*% theta)
  }
  return(f)
}
...
gradient.hcc.r_Normal <- function(theta, ATT = TRUE) {
  eta0 <- 0
  eta1 <- 0
  for (u in 2:r) {
    for (s in 1:(u - 1)) {
  ...
  if (ATT) {
    ...
    for (k in 1:r) {
      f0 <- f0 + sum(drop(W * exp(Ze_Normal[[k]] %*% theta)))
      f1 <- f1 + drop((t(Ze_Normal[[k]]) - eta1 / eta0) %*% drop(W * exp(Ze_Normal[[k]] %*% theta)))
    }
    f <- drop(f1 / f0 - Ze0_bar_al_Normal)
  }
...
gradient.hyj.r_Normal <- function(theta, ATT = TRUE) {
  ...
  if (ATT) {
    ...
    Numerator <- colSums(W * avgmat_Permutation)
    # 注意：simulation比较特殊的设定是每n_i都相同，n_i = r
    for (k in 1:r) {
      if (k == 1) {
        Denominator <- W %*% exp(Ze_Normal[[k]] %*% theta)
      }else {
        Denominator <- Denominator + W %*% exp(Ze_Normal[[k]] %*% theta)
      }
    }
  }
  ...
  if (ATT) {
    ...
    HYJ_val <- Numerator / drop(Denominator) - Ze0_bar_al_Normal
  }
  return(HYJ_val)
}
